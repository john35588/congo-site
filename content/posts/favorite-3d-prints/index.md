---
title: "Some of my Favorite 3D Prints"
date: 2026-02-24
draft: false
description: "I'm often asked what my favorite things to print are, here's my response to that question."
summary: "I'm often asked what my favorite things to print are, here's my response to that question."
slug: "favorite-3d-prints"
tags: ["DIY", "hobby", "3D printing", "3D design"]
---

## Introduction
I started 3D printing over ten years ago. My first 3D printer was a cheap little Monoprice MP Mini Select v2. It worked well for several years, but was too slow, loud, and didn't produce the quality I wanted, so I upgraded a few years later to an Ender 3. 

When I tell people I enjoy 3D printing, the first question I get is either, "what is your favorite thing to print?" or "what's the coolest thing you've printed?" I never know how to answer these questions, so I usually just tell them about something I assume they would think is cool that I printed recently. 

The main reason I don't always know how to respond is because to me, the coolest part about 3D printing isn't all the little trinkets and fidget toys you see so often online like most people assume. No, the coolest part of 3D printing is the excitement I get when I design something on my computer then print it, and it works! The feeling of joy I get when something I design functions properly is impossible to describe. 

In this post, I will share a few projects from the past several years that are among my favorite prints. I will include links to the models when possible.

## 1. Adjustable Bench Power Supply
{{< figure
    src="power-supply.webp"
    alt="Adjustable bench power supply"
    caption="My 3D printed bench power supply"
>}}

This was a fun project from February 2025. I wanted a nice bench power supply, but they're expensive, and I knew I'd likely never use all the features on a nice one anyways. I got the idea from [this instructable](https://www.instructables.com/Lab-Bench-Power-Supply-1/), but I wanted something more compact, and it seemed like a good opportunity to expand my skills. 

The design phase took quite some time for this project. I ended up buying most of the internal parts first, then modeling them and arranging the parts in Fusion before designing the case. 
{{< figure
    src="fusion-screenshot-open.webp"
    alt="View of the power supply with a few panels hidden to expose the inside"
    caption="My design in Fusion with a few panels hidden to expose the internals"
>}}
I tried to make it as compact as possible while maintaining plenty of room for good airflow. The final design includes a 0-50 volt output and two USB ports. I use it mainly for powering small circuits and components.

This project was one of my favorites because I use it often, and it just came together so well! 

Models and instructions for this design are on [Thingiverse](https://www.thingiverse.com/thing:6940739).

